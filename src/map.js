...
const mapHeight = 6.7; // Height of the walls
const wallThickness = 0.1; // Wall thickness
const wallRemoval = 0.65; // Random wall removal proportion
...
function genGrid(segments, n, r) {
  ...
  let gridSegs = Array(cellCount * cellCount)
    .fill()
    .map(() => []);
  let gridParts = Array(cellCount * cellCount)
    .fill()
    .map(() => []);
  ...
  placements = placements.filter(
    ([[ax, ay], [bx, by]]) => Math.hypot(ax - bx, ay - by) > 1
  );
  ...
  const getAreaIndex = (x, y) => {
    let index = areas.findIndex(
      (a) => Math.abs(a[0] - x) < r / 2 && Math.abs(a[1] - y) < r / 2
    );
    if (index === -1)
      // Middle of room => search neighbour cells
      index = areas.findIndex(
        (a) => Math.abs(a[0] - x) + Math.abs(a[1] - y) < r
      );
    return index;
  };
...
module.exports = function (
  ...
  let position = segments.flat().flatMap(([x, y]) => [
    [x, 0, y],
    [x, h, y],
  ]);
  let { getAreaIndex, getGridSegments, getGridParts, placements } = genGrid(
    segments,
    n,
    r
  );
